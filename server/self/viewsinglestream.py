...
class ShowPictures(webapp2.RequestHandler):
    def get(self): 
        ...
        count.put()
        pictures=db.GqlQuery("SELECT * FROM Picture " +"WHERE ANCESTOR IS :1 "+"ORDER BY uploaddate DESC",db.Key.from_path('User',user_name,'Stream',stream_name))
        ...
        template_values={"stream_name": stream_name,"infos":infos,"url":url,"status":status,"sharelink": self.request.url}
...
class MobileStreamAPI(webapp2.RequestHandler):
    def post(self):          
        ...
        streams=Stream.query(Stream.author_name==username).fetch()
        names=[]
        coverurls=[]
        ...
        result=dict()
        result["streamnames"]=names
        result["streamcoverurls"]=coverurls
        result=json.dumps(result)
        self.response.headers['Content-Type']="application/json"
        self.response.write(result)  

class MobilePicAPI(webapp2.RequestHandler):
    def post(self):          
        receivedata=json.loads(self.request.body)
        username=receivedata["username"]
        streamname=receivedata["streamname"]
        
        #stream=Stream.query(Stream.name==username, Stream.author_name==username).fetch()[0]
        pictures=db.GqlQuery("SELECT * FROM Picture " +"WHERE ANCESTOR IS :1 "+"ORDER BY uploaddate DESC",db.Key.from_path('User',username,'Stream',streamname))
        
        data=[]
        for picture in pictures:
            data.append(str(picture.key()))
        
        result=dict()
        result["pictures"]=data
        result=json.dumps(result)
        self.response.headers['Content-Type']="application/json"
        self.response.write(result)

class MobileNearbyAPI(webapp2.RequestHandler):
    def post(self): 
        receivedata=json.loads(self.request.body)
        
        print receivedata
        username=receivedata["username"]
        latitude=float(receivedata["latitude"])
        longitude=float(receivedata["longitude"])
        
        streams=Stream.query(Stream.author_name==username)
        datastream=[]
        datapic=[]
        for stream in streams:
            pictures=db.GqlQuery("SELECT * FROM Picture " +"WHERE ANCESTOR IS :1 "+"ORDER BY uploaddate DESC",db.Key.from_path('User',username,'Stream',stream.name))
            for picture in pictures:
                if(abs(picture.latitude-latitude)<0.001 and abs(picture.longitude-longitude)<0.001):
                    #data.append((stream.name,str(picture.key())))
                    datastream.append(stream.name)
                    datapic.append(str(picture.key()))
...
application = webapp2.WSGIApplication([
    ...
    ('/geoapi',GeoAPI),
    ('/picapi',PicAPI), 
    ('/mobilestream',MobileStreamAPI),
    ('/mobilepic',MobilePicAPI),
    ('/mobilenearby',MobileNearbyAPI),
], debug=True)